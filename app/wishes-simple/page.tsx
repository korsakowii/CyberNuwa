...
export default function WishesSimple() {
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'idea': return language === 'zh' ? '灵感' : 'Idea'
    ...
    <div className="min-h-screen bg-zinc-900 text-white">
      <div className="max-w-6xl mx-auto px-4 py-10">
        <div className="text-center mb-12">
          <a className="text-zinc-400 hover:text-white transition-colors mb-4 inline-block" href="/">
          ...
          <p className="text-zinc-400 mb-6">
            {language === 'zh' ? '展示灵感碎片和半成品想法' : 'Show inspiration fragments and half-finished ideas'}
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {wishes.map((wish) => (
            <div key={wish.id} className="bg-zinc-800/50 backdrop-blur-sm border border-zinc-700 rounded-xl p-6 hover:bg-zinc-800/70 hover:border-zinc-600 transition-all duration-300 transform hover:-translate-y-1">
