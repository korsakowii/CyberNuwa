        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {modules.map((module, index) => (
            <Link 
              key={module.href}
              href={module.href}
              className="block group transform transition-all duration-300 hover:-translate-y-2 hover:scale-105"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <div className='bg-zinc-800/50 backdrop-blur-sm border border-zinc-700 rounded-xl p-6 h-full transition-all duration-300 hover:bg-zinc-800/70 hover:border-zinc-600 cursor-pointer'>
                <div
                  className={`w-12 h-12 rounded-lg bg-gradient-to-r ${module.color} flex items-center justify-center text-2xl mb-4 group-hover:scale-110 transition-transform duration-300`}
                >
                  {module.icon}
                </div>
